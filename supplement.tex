...
Section~\ref{sec:settings} lists the training settings and the sample counts of the case studies.
...
The sixth-order Sauer--Pai model \cite{sauer2018} with the GENROU saturation \cite{cui2021} is restated from \eqref{M-eq:sm} of the main text, so that the variants of Sections~\ref{ssec:marconato} and \ref{ssec:reduced} can refer to it, and reads
...
\end{align}
\end{subequations}
The symbols of the main text keep their meaning.
...
\begin{proof}
Write \eqref{M-eq:closure} as $\dot{\bm{x}}_r=\bm{\Lambda}\bm{x}_r+\bm{p}(t)$ with $\bm{\Lambda}=\bm{T}^{-1}A$ and $\bm{p}=\bm{T}^{-1}\bigl(B\bm{u}_r+\bm{r}_{\bm{\theta}}\bigr)$.
The output bound of \eqref{M-eq:rtheta} gives $\lvert r_{\bm{\theta},i}\rvert\le c_i$, so $\lVert\bm{p}(t)\rVert_2\le\bar{u}+\lVert\bm{T}^{-1}\bm{c}\rVert_2$ under the input bound $\lVert\bm{T}^{-1}B\bm{u}_r(t)\rVert_2\le\bar{u}$.
Every eigenvalue of $\bm{\Lambda}$ is negative, so constants $\gamma\ge1$ and $\lambda>0$ with $\lVert e^{\bm{\Lambda}t}\rVert_2\le \gamma e^{-\lambda t}$ exist \cite{khalil2002}.
The variation-of-constants formula gives $\bm{x}_r(t)=e^{\bm{\Lambda}t}\bm{x}_r(0)+\int_0^{t}e^{\bm{\Lambda}(t-\tau)}\bm{p}(\tau)\,\mathrm{d}\tau$.
Bounding the integrand by $\gamma e^{-\lambda(t-\tau)}\lVert\bm{p}\rVert_2$ and integrating yields \eqref{M-eq:ultbound}.
\end{proof}
...
Write $\bm{w}=[\bm{x}^\top,\bm{y}^\top]^\top$ for the full unknown vector of the DAE \eqref{M-eq:dae}, $\bm{F}=[\bm{f}^\top,\bm{g}^\top]^\top$ for its right-hand side, and $M=\operatorname{diag}(I,0)$ for the mass matrix, so that the DAE is $M\dot{\bm{w}}=\bm{F}(t,\bm{w})$.
...
M\Bigl(\sum_{m=1}^{k}\frac{1}{m}\nabla^m\bm{w}_{n+1}-\kappa\gamma_k\nabla^{k+1}\bm{w}_{n+1}\Bigr)
=h\bm{F}(t_{n+1},\bm{w}_{n+1}),
\end{equation}
where $h$ is the step size, $\nabla$ is the backward difference operator with $\nabla^0\bm{w}_n=\bm{w}_n$ and $\nabla^{j+1}\bm{w}_n=\nabla^j\bm{w}_n-\nabla^j\bm{w}_{n-1}$, $\gamma_k=\sum_{j=1}^{k}1/j$, and $\kappa$ is the damping coefficient that lowers the leading truncation term.
With the predictor $\bm{w}_{n+1}^{(0)}=\sum_{m=0}^{k}\nabla^m\bm{w}_n$, equation \eqref{eq:ndf} becomes the residual equation \eqref{M-eq:ndf-resid} of the main text,
\begin{equation}\label{eq:ndf-resid}
\begin{aligned}
\bm{G}(\bm{w})&=M\bigl(\bm{w}-\bm{w}_{n+1}^{(0)}+\bm{\Psi}\bigr)-\alpha_k\bm{F}(t_{n+1},\bm{w})=\bm{0},\\
...
\section{Training Settings and Sample Counts}\label{sec:settings}

The correction network \eqref{M-eq:rtheta} has two hidden layers of 64 units with the SiLU activation, and its output layer is zero-initialized.
The bound is $\bm{c}=\bm{1}$, raised to $\bm{c}=3\cdot\bm{1}$ for the GENROU settings so that the saturation can be absorbed.
In the loss \eqref{M-eq:loss}, the current and the speed are normalized by the standard deviation of the recorded quantity, the weight is $\mu=1$, and the samples of the fault-on period are weighted by 3.
Training uses AdamW for 1500 steps with a learning rate of $6\times10^{-3}$ for the weights and the parameters, $3\times10^{-2}$ for the latent $\delta_0$, a linear warm-up over the first $5\%$ of the steps, and a cosine decay to $2\%$ of the peak.
Each step draws 128 windows of 2.5~s that start at the pre-fault steady state, and the surrogate is integrated by the fixed-step fourth-order Runge--Kutta scheme at the 1~ms sampling interval of the measurements.
Table~\ref{tab:samples} lists the sample counts.
The clearing times of the IEEE 14-bus study run from $0.10$ to $0.70$~s in steps of $0.03$~s, and those of the NPCC and 2224-bus studies form a grid across the critical clearing time of each scenario.
One full-physics run supplies one trajectory for every recorded machine, so the NPCC runs are shared by the twelve surrogated machines and the runs of each 2224-bus family by its trained members.

\begin{table}[!h]
\caption{Sample Counts of the Case Studies}
\label{tab:samples}
\centering\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccc}
\toprule
Study & Scenarios & Loading levels & Runs\\
\midrule
IEEE 14-bus & 6 & 7 & 882\\
IEEE 14-bus, GENROU & 6 & 4 & 504\\
NPCC & 7 & 5 & 745\\
2224-bus & 6 & 1 & 112\\
\bottomrule
\end{tabular}
...
\section{Fault Scenarios of the NPCC and 2224-Bus Studies}\label{sec:scenarios}

Table~\ref{tab:scenarios} lists the fault scenarios of Figs.~\ref{M-fig:cct_npcc} and \ref{M-fig:cct_gb} of the main text.
Every scenario is a three-phase fault at the bus in the column Fault, cleared by tripping the branch between the two buses in the column Branch.
The label carries the faulted bus after F and the index of the tripped branch in the case file after L.
The branch of F87-L230 is a transformer, and every other branch is a line.

\begin{table}[!h]
\caption{Fault Scenarios of the Two Large Systems}
\label{tab:scenarios}
\centering\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{lcclcc}
\toprule
\multicolumn{3}{c}{NPCC} & \multicolumn{3}{c}{2224-bus}\\
\cmidrule(lr){1-3}\cmidrule(lr){4-6}
Scenario & Fault & Branch & Scenario & Fault & Branch\\
\midrule
F85-L108 & 85 & 85--112 & F316-L302 & 316 & 316--163\\
F87-L230 & 87 & 87--85 & F14-L308 & 14 & 14--344\\
F115-L157 & 115 & 115--135 & F333-L293 & 333 & 333--98\\
F133-L201 & 133 & 133--135 & F316-L260 & 316 & 316--98\\
F135-L204 & 135 & 135--139 & F130-L316 & 130 & 130--350\\
F131-L195 & 131 & 131--135 & F318-L1427 & 318 & 318--356\\
F132-L197 & 132 & 132--133 & F318-L1428 & 318 & 318--357\\
 & & & F40-L272 & 40 & 40--320\\
\bottomrule
\end{tabular}
\end{table}
